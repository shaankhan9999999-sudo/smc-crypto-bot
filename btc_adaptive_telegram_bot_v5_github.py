...
def detect_liquidity_sweep(df):
    ...
    if (
        ...
        high_distance_atr = (
            high_penetration / current_atr
        )

        low_distance_atr = (
            low_penetration / current_atr
        )
        ...
        return {
            ...
            "level": (
                previous_high
                if high_distance_atr >= low_distance_atr
                else previous_low
            ),
...
def impulse_retest_ev(
    ...
    if (
        imp
        and imp.get("direction") == direction
    ):
...
def sweep_confirmation(
    ...
    # IMPORTANT BUG FIX:
    #
    # A sweep result is a dictionary.
    # {"type": "NONE"} is therefore truthy in Python.
    #
    # The old code could accidentally treat NONE as an
    # opposite sweep and block the trade.
    #
    # NONE = no sweep = sweep gate inactive.
    # --------------------------------------------------------

    if (
        not sweep
        or sweep.get("type", "NONE") == "NONE"
    ):
    ...
    return {
        ...
        "reason": (
            "confirmed"
            if confirmation >= SWEEP_MIN_CONFIRMATION
...
def format_sweep(
    ...
    distance = safe_float(
        sweep.get("distance_atr")
    )

    reclaim = safe_float(
        sweep.get("reclaim_strength")
    )
    ...
    return (
        ...
        f"Reclaim: {reclaim * 100:.0f}%\n"
        f"Quality: {quality * 100:.0f}%\n"
        f"Confirmation: {conf * 100:.0f}%\n"
        f"Sweep status: {reason}"
    )


# ============================================================
# FORMAT SWEEP DIAGNOSTIC TEXT
# ============================================================

def format_sweep_diagnostic(
    diagnostic,
):

    if not diagnostic:
        return "Sweep Debug: unavailable"

    raw_type = diagnostic.get(
        "raw_type",
        "NONE",
    )

    active = diagnostic.get(
        "active",
        False,
    )

    direction = diagnostic.get(
        "direction"
    )
    ...
    conf_reason = diagnostic.get(
        "confirmation_reason",
        "n/a",
    )

    blocked = diagnostic.get(
        "blocked",
        False,
    )
    ...
    return (
        ...
        f"Age: {age_text}\n"
        f"Distance: {distance_atr:.3f} ATR\n"
        f"Reclaim: {reclaim * 100:.1f}%\n"
        f"Quality: {quality * 100:.1f}%\n"
        f"Confirmation: {confirmation_pct:.1f}%\n"
        f"Required: {threshold_pct:.1f}%\n"
        f"Conf Reason: {conf_reason}\n"
        f"Sweep Blocked: {blocked}"
    )
...
def score_direction(
    ...
    # Prepare each timeframe independently.
    m15 = prepare(
        F["15m"]
    )
    ...
    # Order-flow snapshot is common for the scan.
    flow_data = flow_ev(
        safe_float(
            m15.iloc[-1]["close"]
        )
    ...
    # --------------------------------------------------------
    # Trend uses higher timeframes + 15m context.
    # --------------------------------------------------------

    trend_values = [
        trend_ev(h4, direction),
        trend_ev(h2, direction),
        trend_ev(m15, direction),
    ]
    ...
    # Use a copy so LONG/SHORT calculations cannot
    # accidentally mutate the same impulse state.
    local_state = copy.deepcopy(
        state
    )
...
def risk_plan(
    ...
    # Existing V5 risk structure:
    # Stop = 1.25 ATR
    # Target = 2.00 ATR
    #
    # This gives RR = 2 / 1.25 = 1.60.
...
def build_signal(
    ...
    long_eval = score_direction(
        F,
        "LONG",
        long_state,
    )

    short_eval = score_direction(
        F,
        "SHORT",
        short_state,
    )
    ...
    # Higher score wins.
    # Exact tie remains LONG, preserving existing behavior.
    ...
    # --------------------------------------------------------
    # Sweep confirmation is a gate only.
    #
    # NO SWEEP:
    #   Existing V5 signal can still pass.
    #
    # VALID SWEEP:
    #   Must confirm selected direction.
    #
    # OPPOSITE / DOUBLE / WEAK:
    #   Can block.
    # --------------------------------------------------------
    ...
    #
    # IMPORTANT:
    # If sweep is NONE, sweep_conf["active"] is False,
    # so it does NOT block the signal.
    # --------------------------------------------------------
    ...
    # If a meaningful sweep is confirmed, expose that in
    # the signal strength label without changing score.
    ...
    # Signal ID.
    #
    # It changes with direction + candle timestamp so that
    # repeated identical scans can be recognized.
    # --------------------------------------------------------
...
def format_signal(sig):
    ...
    sweep_debug_text = format_sweep_diagnostic(
        sig.get(
            "sweep_diagnostic",
            {},
        )
...
def run_once():
    ...
    if same or cooldown:
        ...
        telegram(
            ...
            f"{sig['score']:.1f}\n"
            f"Reason: {reason}\n\n"
            f"{format_sweep_diagnostic(sig.get('sweep_diagnostic', {}))}"
        )
