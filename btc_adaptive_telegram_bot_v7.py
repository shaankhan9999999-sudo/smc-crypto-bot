...
def _sweep_at(d,i):
    ...
    if bs and ss:
        hp=(r.high-ph)/atrv; lp=(pl-r.low)/atrv; rec=max((ph-r.close)/rng,(r.close-pl)/rng); return {"type":"DOUBLE_SWEEP","level":ph if hp>=lp else pl,"distance_atr":max(hp,lp),"reclaim_strength":clamp(rec),"quality":clamp(.5*clamp(max(hp,lp)/.5)+.5*clamp(rec)),"age":0,"direction":None,"candle_ts":r.ts.isoformat()}
    if ss:
        pen=(pl-r.low)/atrv; rec=clamp((r.close-pl)/rng); rej=clamp((min(r.open,r.close)-r.low)/rng); return {"type":"SELL_SIDE_SWEEP","level":pl,"distance_atr":pen,"reclaim_strength":rec,"quality":clamp(.4*clamp(pen/.5)+.4*rec+.2*rej),"age":0,"direction":"LONG","candle_ts":r.ts.isoformat()}
    pen=(r.high-ph)/atrv; rec=clamp((ph-r.close)/rng); rej=clamp((r.high-max(r.open,r.close))/rng); return {"type":"BUY_SIDE_SWEEP","level":ph,"distance_atr":pen,"reclaim_strength":rec,"quality":clamp(.4*clamp(pen/.5)+.4*rec+.2*rej),"age":0,"direction":"SHORT","candle_ts":r.ts.isoformat()}
...
def build_signal(F,state,news,fd):
    le=score_direction(F,"LONG",state,fd); se=score_direction(F,"SHORT",state,fd); best=se if se["score"]>le["score"] else le; direction=best["direction"]; vals=best["vals"];
    sweep=detect_liquidity_sweep(F["1h"]); sc=sweep_confirmation(direction,sweep,vals,fd); cont=best["continuation"]; plan=risk_plan(best["prepared"]["15m"],direction); reasons=[]
    ...
    if sc.get("active") and sc.get("confirmation",0)<SWEEP_MIN_CONFIRMATION: reasons.append("double_sweep_unclear" if sc.get("reason")=="double_sweep_direction_unclear" else "sweep_not_confirmed")
    news_status=news_alignment(news,direction)
    if reasons: return None,{"direction":direction,"score":best["score"],"agreement":best["agreement"],"vals":vals,"flow":fd,"plan":plan,"sweep":sweep,"sweep_confirmation":sc,"continuation":cont,"news":news,"news_alignment":news_status,"reason":",".join(reasons)}
    strength="STRONG" if best["score"]>=STRONG_SCORE and best["agreement"]>=STRONG_AGREEMENT else "SIGNAL"
    if sc.get("reason")=="confirmed": strength="SWEEP_CONFIRMED_"+strength
    if cont.get("state")=="CONTINUATION_READY": strength="CONTINUATION_"+strength
    if news_status=="SUPPORTIVE" and news.get("confirmation",0)>=.75: strength="NEWS_CONFIRMED_"+strength
    ts=best["prepared"]["15m"].iloc[-1].ts.isoformat(); sid=f"{direction}_{ts}"
    sig={"id":sid,"ts":iso(),"direction":direction,"strength":strength,"score":best["score"],"agreement":best["agreement"],"vals":vals,"flow":fd,"plan":plan,"sweep":sweep,"sweep_confirmation":sc,"continuation":cont,"news":news,"news_alignment":news_status}
    return sig,{"direction":direction,"score":best["score"],"agreement":best["agreement"],"vals":vals,"flow":fd,"plan":plan,"sweep":sweep,"sweep_confirmation":sc,"continuation":cont,"news":news,"news_alignment":news_status,"reason":"all_gates_pass"}
...
def format_signal(sig):
    ...
    return (f"₿ BTC ADAPTIVE V7 — {sig['strength']} {sig['direction']}\n\nSignal ID: {sig['id']}\nEntry: {p['entry']:.2f}\nSL: {p['stop']:.2f}\nTP: {p['target']:.2f}\nRR: {p['rr']:.2f}\n\nScore: {sig['score']:.1f}/{sum(WEIGHTS.values())}\nAgreement: {sig['agreement']*100:.0f}%\n\nTrend {v['trend']:.2f} | Structure {v['structure']:.2f}\nPrice {v['price']:.2f} | Vol {v['volume']:.2f}\nMomentum {v['momentum']:.2f} | Retest {v['retest']:.2f}\nReversal {v['reversal']:.2f} | OF {v['orderflow']:.2f}\n\n{sweep_text(sig['sweep'],sig['sweep_confirmation'])}\n\nPost-Impulse: {c.get('state')} | Score {c.get('score',0)*100:.0f}%\nPullback: {c.get('pullback_depth',0)*100:.0f}% | Reversal risk: {c.get('reversal_risk',0)*100:.0f}%\n\nNews: {sig['news_alignment']} | confirmation {n.get('confirmation',0)*100:.0f}% | events {n.get('count',0)}" + (f"\nHeadline: {headline[:180]}" if headline else "") + f"\n\nUTC: {sig['ts']}")
...
def register_trade(state,sig,msg_id):
    state.setdefault("active_trades",{})[sig["id"]]={"signal_id":sig["id"],"direction":sig["direction"],"entry":sig["plan"]["entry"],"stop":sig["plan"]["stop"],"target":sig["plan"]["target"],"initial_target":sig["plan"]["target"],"target_version":0,"status":"ACTIVE","created_at":now_ts(),"last_update_at":now_ts(),"last_update_price":sig["plan"]["entry"],"telegram_message_id":msg_id,"score":sig["score"],"agreement":sig["agreement"],"news_alignment":sig["news_alignment"],"continuation":sig["continuation"],"directional_snapshot":sig["vals"]}
...
def manage_active_trades(state,F,news,fd):
    ...
    for sid,trade in list(state["active_trades"].items()):
        ...
        if can and now_ts()-safe_float(trade.get("last_update_at",0))>EXTENSION_COOLDOWN_MIN*60:
            old=target; trade["target"]=can["new_target"]; trade["target_version"]=int(trade.get("target_version",0))+1; trade["status"]="TARGET_EXTENDED"; trade["last_update_at"]=now_ts(); trade["last_update_price"]=price; telegram(f"🔄 BTC TRADE UPDATE\n\nSignal ID: {sid}\nThis is NOT a new signal. It is an update to the existing {d} setup.\n\nTARGET EXTENSION #{trade['target_version']}\nPrevious TP: {old:.2f}\nNew TP: {trade['target']:.2f}\nReason: {can['reason']}\nContinuation: {cont['state']}",trade.get("telegram_message_id")); journal("TARGET_EXTENDED",signal_id=sid,previous_target=old,new_target=trade["target"],trade=trade)

def wait_message(det):
    n=det.get("news",{}); c=det.get("continuation",{}); return (f"₿ BTC ADAPTIVE V7 — WAIT\n\nBest: {det['direction']}\nScore: {det['score']:.1f}/{sum(WEIGHTS.values())}\nAgreement: {det['agreement']*100:.0f}%\nReason: {det['reason']}\n\n{sweep_text(det.get('sweep',{}),det.get('sweep_confirmation',{}))}\n\nPost-Impulse: {c.get('state','NONE')} | {c.get('score',0)*100:.0f}%\nNews: {det.get('news_alignment','NEUTRAL')} | {n.get('confirmation',0)*100:.0f}%\nOrderFlow: {det['flow'].get('text','n/a')}")
...
def run_once():
    ...
    last=safe_float(state.get("last_signal_ts")); same=state.get("last_signal")==sig["id"]; cooldown=state.get("last_direction")==sig["direction"] and now_ts()-last<COOLDOWN_MIN*60
    if same or cooldown:
        reason="same_signal" if same else "same_direction_cooldown"; telegram(f"₿ BTC V7 — SIGNAL SUPPRESSED\n{sig['direction']} {sig['score']:.1f}\nReason: {reason}\nSignal ID: {sig['id']}"); journal("SUPPRESSED",signal=sig,reason=reason); save_state(state); return
    msgid=telegram(format_signal(sig)); journal("SIGNAL",signal=sig,telegram_message_id=msgid,flow_ok=flow_ok); register_trade(state,sig,msgid); state["last_signal"]=sig["id"]; state["last_signal_ts"]=now_ts(); state["last_direction"]=sig["direction"]; csv_log({"timestamp":sig["ts"],"event":"SIGNAL","signal_id":sig["id"],"direction":sig["direction"],"score":sig["score"],"agreement":sig["agreement"],"entry":sig["plan"]["entry"],"sl":sig["plan"]["stop"],"tp":sig["plan"]["target"],"rr":sig["plan"]["rr"],"news_status":news.get("status"),"news_direction":news.get("direction"),"news_confirmation":news.get("confirmation"),"news_impact":news.get("impact"),"news_confidence":news.get("confidence"),"news_relevance":news.get("relevance"),"news_event_count":news.get("count"),"continuation_state":sig["continuation"].get("state"),"continuation_score":sig["continuation"].get("score"),"pullback_depth":sig["continuation"].get("pullback_depth"),"breakout_level":sig["continuation"].get("breakout_level"),"reversal_risk":sig["continuation"].get("reversal_risk"),"lifecycle":"NEW","reason":sig["strength"]}); save_state(state)
