...
def send_telegram_message(message):
    ...
    payload = {
        ...
        "text": message
        # Plain text mode रखा है ताकि Markdown की वजह से मैसेज फेल न हो
    }
    try:
...
def fetch_coinbase_klines(product_id="BTC-USD", granularity=60):
    ...
    try:
        print(f"Fetching Coinbase data for {product_id} (granularity={granularity})...")
        response = requests.get(url, headers=headers, timeout=10)
        data = response.json()
        
        if not isinstance(data, list) or len(data) == 0:
            print(f"Coinbase API error or empty data: {data}")
            return pd.DataFrame()
...
def run_scalping_bot():
    print("Starting Scalping Bot Scan...")
