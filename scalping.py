...
def fetch_mexc_klines(symbol="BTCUSDT", interval="1m", limit=100):
    url = f"https://api.mexc.com/api/v3/klines?symbol={symbol}&interval={interval}&limit={limit}"
    try:
        response = requests.get(url, timeout=10)
        data = response.json()
        if not isinstance(data, list) or len(data) == 0:
            print("MEXC API error or empty data")
            return pd.DataFrame()
...
def run_scalping_bot():
    df_5m = fetch_mexc_klines("BTCUSDT", interval="5m", limit=100)
    df_1m = fetch_mexc_klines("BTCUSDT", interval="1m", limit=100)
    ...
    if df_5m is None or df_5m.empty or df_1m is None or df_1m.empty or len(df_5m) < 21 or len(df_1m) < 21:
        ...
        send_telegram_message("🤖 *[SCALP BOT]*\nWarning: Market data from MEXC is empty or insufficient.")
        return
    ...
    if bullish_scalp:
        ...
        msg = (
            f"🤖 *[SCALP BOT STATUS]*\n"
            f"Market Scanned (MEXC): No trade setup found.\n"
