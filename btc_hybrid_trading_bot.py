...
def send_telegram_message(message):
  ...
  try:
    ...
    if res_data.get("ok"):
      print("📤 Bot #3 Dual-Volume Telegram Alert Delivered Successfully!")
      return True
    else:
...
def run_balanced_multitimeframe_bot():
  print("=" * 60)
  print(
      "🤖 [BOT #3 ACTIVE] Running Hybrid Multi-Timeframe Bot (Resampled"
      " Accuracy System)..."
  )
  print("=" * 60)
  ...
  # इससे हमेशा पर्याप्त और सटीक 2H कैंडल्स मिलेंगी, डेटा की कोई कमी नहीं होगी
  df_temp = df_1h.copy()
  ...
  # अब यहाँ हमेशा पक्की और सटीक 6 कैंडल्स मिलेंगी (यानी मजबूत मैक्रो ज़ोन)
  ...
  print(f"📊 [MARKET STATS - BOT #3 RESAMPLED HYBRID]")
  ...
  is_heavy_volume = current_vol >= (
      2.0 * avg_vol
  )  # Tier 1: Heavy Explosive Volume (>= 2.0x)
  is_above_average = (
      current_vol > avg_vol
  )  # Tier 2: Standard Above-Average Volume (> 1.0x)

  # --- 1. Heavy Volume Breakout Strategy (Tier 1 - High Probability) ---
  ...
  if is_heavy_volume and (candle_body > (1.1 * current_atr)):
    if current_price > current_open and current_rsi < 82:
      ...
      msg = (
          ...
          f"🎯 *Target:* `{target:.2f}`\n"
          f"═══════════════════════\n"
          f"💡 *Bot Source:* Bot #3 (Resampled Hybrid)\n"
          f"📊 *Volume Status:* `HEAVY SPIKE (>= 2.0x)` 🚀\n"
          f"📌 *Probability:* High Confidence Setup\n"
  ...
  # --- 2. Smart Hybrid Zone & Wick Sweep Setup (Tier 2 - Above Average Volume) ---
  print("🔎 Checking Tier-2 Above-Average Volume Hybrid Setup...")
  ...
  if bullish_signal:
    print("📈 Bot #3 Tier-2 Above-Average Volume Long Triggered!")
    stop_loss = current_price - (1.8 * current_atr)
    target = (
        macro_resistance
        if macro_resistance > current_price
    ...
    print("📉 Bot #3 Tier-2 Above-Average Volume Short Triggered!")
    stop_loss = current_price + (1.8 * current_atr)
    target = (
        macro_support
        if macro_support < current_price
  ...
  print(
      "❌ No valid dual-volume signal generated in this scan cycle for Bot #3."
  )
  print("=" * 60)
