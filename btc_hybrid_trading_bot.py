...
def run_balanced_multitimeframe_bot():
  ...
  # --- 2. Smart Hybrid Zone & Wick Sweep Setup (Tier 2) ---
  ...
  # --- 3. Low-Volume Pullback Continuation Setup (Tier 3 - NEW) ---
  print("🔎 Checking Tier-3 Low-Volume Pullback / Healthy Retest Setup...")

  # पिछले 4 कैंडल्स में वॉल्यूम स्पाइक चेक करने के लिए
  recent_volumes = df_1h["Volume"].iloc[-5:-1]
  has_recent_spike = (recent_volumes >= (1.5 * avg_vol)).any()

  # लो वॉल्यूम कंडीशन (करंट वॉल्यूम एवरेज से कम है लेकिन शून्य नहीं)
  is_low_volume_pullback = (current_vol < avg_vol) and (
      current_vol > (0.3 * avg_vol)
  )

  # लॉन्ग और शॉर्ट पुलबैक की शर्तें
  pullback_long_cond = (
      has_recent_spike
  ...
  if pullback_long_cond:
    print(
        "📈 Bot #3 Tier-3 Low-Volume Pullback LONG Triggered (Healthy Retest)!"
    )
    stop_loss = current_price - (1.8 * current_atr)
    target = (
        macro_resistance
    ...
    msg = (
        f"🌊 *[ALERT: THIRD BOT - LOW VOL PULLBACK LONG]* 🌊\n"
        f"═══════════════════════\n"
        f"🪙 *Asset:* `BTC-USD`\n"
        f"🟢 *Signal:* BUY / LONG\n"
        f"📍 *Entry Price:* `{current_price:.2f}`\n"
        ...
        f"═══════════════════════\n"
        f"💡 *Bot Source:* Bot #3 (Smart Hybrid)\n"
        f"📊 *Volume Status:* `LOW VOL PULLBACK (< avg)` 🌊\n"
        f"📌 *Setup:* Post-Spike Healthy Retest\n"
        f"📈 *RSI:* `{current_rsi:.1f}`"
    )
    send_telegram_message(msg)
    return

  elif pullback_short_cond:
    print(
        "📉 Bot #3 Tier-3 Low-Volume Pullback SHORT Triggered (Healthy Retest)!"
    )
    stop_loss = current_price + (1.8 * current_atr)
    target = (
        macro_support
    ...
    msg = (
        f"🌊 *[ALERT: THIRD BOT - LOW VOL PULLBACK SHORT]* 🌊\n"
        f"═══════════════════════\n"
        f"🪙 *Asset:* `BTC-USD`\n"
        f"🔴 *Signal:* SELL / SHORT\n"
        f"📍 *Entry Price:* `{current_price:.2f}`\n"
