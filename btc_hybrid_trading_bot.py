...
def run_balanced_multitimeframe_bot():
  print("=" * 60)
  print(
      "🤖 [BOT #3 ACTIVE] Running Hybrid Multi-Timeframe Bot (Dual-Volume"
      " System)..."
  )
  print("=" * 60)

  # --- 2H Macro Zones (Timeframe: 2 Hours, Min Candles: 6) ---
  ...
  print(f"📊 [MARKET STATS - BOT #3 DUAL-VOLUME HYBRID]")
  ...
  if is_heavy_volume and (candle_body > (1.1 * current_atr)):
    if current_price > current_open and current_rsi < 82:
      ...
      msg = (
          ...
          f"🎯 *Target:* `{target:.2f}`\n"
          f"═══════════════════════\n"
          f"💡 *Bot Source:* Bot #3 (Dual-Volume Hybrid)\n"
  ...
  # Trigger requires valid pattern AND volume must be at least above average (> 1.0x)
  bullish_signal = (
