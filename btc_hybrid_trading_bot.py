...
def send_telegram_message(message):
  ...
  try:
    ...
    if res_data.get("ok"):
      print("📤 Hybrid Bot Telegram Alert Delivered Successfully!")
      return True
    else:
...
def run_balanced_multitimeframe_bot():
  ...
  # --- 2H Macro Zones (Timeframe: 2 Hours, Min Candles reduced to 10 for safety) ---
  df_2h = fetch_coinbase_klines("BTC-USD", granularity=7200)
  if df_2h.empty or len(df_2h) < 10:
    print("❌ Insufficient 2H macro data.")
    return

  macro_window = df_2h.iloc[-10:-1]
  ...
  if volume_spike and (candle_body > (1.2 * current_atr)):
    if current_price > current_open and current_rsi < 82:
      ...
      msg = (
          ...
          f"🎯 *Target:* `{target:.2f}`\n"
          f"━━━━━━━━━━━━━━━━━━━\n"
          f"💡 *Details:* Volume Spike + Safe RSI ({current_rsi:.1f})"
      )
  ...
  if bullish_signal:
    ...
    # Distinct Telegram Notification for Bot #3
    msg = (
        f"🛡️ *[BOT #3: HYBRID SYSTEM LONG]* 🛡️\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"🪙 *Asset:* `BTC-USD`\n"
        f"🟢 *Signal:* BUY / LONG\n"
        f"📍 *Entry Price:* `{current_price:.2f}`\n"
    ...
    # Distinct Telegram Notification for Bot #3
    msg = (
        f"🛡️ *[BOT #3: HYBRID SYSTEM SHORT]* 🛡️\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"🪙 *Asset:* `BTC-USD`\n"
        f"🔴 *Signal:* SELL / SHORT\n"
        f"📍 *Entry Price:* `{current_price:.2f}`\n"
