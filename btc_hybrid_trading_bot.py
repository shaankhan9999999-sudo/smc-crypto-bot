...
def run_balanced_multitimeframe_bot():
  print("=" * 60)
  print(
      "🔍 [MTF SCAN START] Running Balanced Multi-Timeframe Bot (4H Macro Zones"
      " + 1H Execution)..."
  )
  print("=" * 60)

  # --- BALANCE POINT 1: Zone Filter (4H for Macro S/R Zones) ---
  df_4h = fetch_coinbase_klines("BTC-USD", granularity=14400)  # 4-Hour Candles
  if df_4h.empty or len(df_4h) < 30:
    print("❌ Insufficient 4H macro data.")
    return
  ...
  print(f" • 4H Macro Resistance : {macro_resistance:.2f}")
  print(f" • 4H Macro Support    : {macro_support:.2f}")
  ...
  # Wick sweeps at 4H Macro S/R zones (Acts as fast reversal triggers)
  ...
  if bullish_signal:
    ...
    reason = (
        "4H Macro Support Wick Sweep (Instant Reversal)"
    ...
    reason = (
        "4H Macro Resistance Wick Sweep (Instant Reversal)"
        if top_wick_sweep
        else "1H EMA Trend Crossover"
    )
