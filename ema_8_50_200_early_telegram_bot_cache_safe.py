#!/usr/bin/env python3
import os, sys, json, lzma, struct, time
...
def coinbase(tf, needed=430):
    ...
    retu# ---------------- XAUUSD: Dukascopy tick feed with persistent candle cache ----------------
# The cache stores native XAUUSD M5/M15/H1 candles, not PAXG or derived BTC data.
# First run downloads the historical warm-up window. Later runs download only a
# small rolling tick window and update the cached candles. This is deliberately
# conservative with Dukascopy: low concurrency, retries, backoff, and no request storm.

DUKA_WORKERS = max(1, min(int(os.getenv('DUKA_WORKERS', '2')), 3))
DUKA_RETRIES = max(1, int(os.getenv('DUKA_RETRIES', '4')))
DUKA_REQUEST_GAP = max(0.15, float(os.getenv('DUKA_REQUEST_GAP', '0.40')))
XAU_INCREMENTAL_HOURS = max(2, int(os.getenv('XAU_INCREMENTAL_HOURS', '3')))
XAU_CACHE_DIR = Path(os.getenv('XAU_CACHE_DIR', 'xau_cache'))
XAU_CACHE_DIR.mkdir(parents=True, exist_ok=True)

def duka_url(dt):
    # Dukascopy datafeed uses a zero-based month in this path.
    return f'https://datafeed.dukascopy.com/datafeed/{DUKA_SYMBOL}/{dt.year:04d}/{dt.month-1:02d}/{dt.day:02d}/{dt.hour:02d}h_ticks.bi5'

def duka_hour(dt):
    url=duka_url(dt)
    last_error=None
    for attempt in range(DUKA_RETRIES):
        try:
            # Small deterministic delay prevents a burst when several workers start.
            if attempt:
                time.sleep(DUKA_REQUEST_GAP * (2 ** (attempt - 1)))
            else:
                time.sleep(DUKA_REQUEST_GAP)
            ...
            if r.status_code==429:
                ...
                if retry_after:
                    try: time.sleep(min(30.0, max(1.0, float(retry_after))))
            ...
            if len(raw)%20:
                raise RuntimeError(f'bad bi5 size {len(raw)}')
            ...
            for ms,ask_i,bid_i,av,bv in struct.iter_unpack('>Iiiii',raw):
                ...
                bid=bid_i/1000.0
                if ask>0 and bid>0:
                    out.append((dt+timedelta(milliseconds=ms),(ask+bid)/2.0))
            return out

        except lzma.LZMAError as e:
            last_error=f'incomplete/corrupt bi5: {e}'
        except Exception as e:
            last_error=str(e)

        # Longer cooldown after a failed request, especially after a 429.
        if attempt < DUKA_RETRIES-1:
            delay=min(30.0, DUKA_REQUEST_GAP * (2 ** attempt))
            time.sleep(delay)

    raise RuntimeError(last_error or 'Dukascopy request failed')

def _cache_file(tf):
    return XAU_CACHE_DIR / f'{tf.lower()}.csv'

def _read_xau_cache(tf):
    f=_cache_file(tf)
    if not f.exists():
    ...
    try:
        ...
        log(f'XAU cache {tf} read failed: {e}')
        return pd.DataFrame(columns=['open','high','low','close','volume'])

def _write_xau_cache(tf,d):
    f=_cache_file(tf)
    x=d.copy().sort_index()
    x.index.name='ts'
    x.reset_index().to_csv(f,index=False)

def _ticks_to_frames(ticks):
    if not ticks:
        return {}
    d=pd.DataFrame(ticks,columns=['ts','price'])
    d['ts']=pd.to_datetime(d['ts'],utc=True)
    d['price']=pd.to_numeric(d['price'],errors='coerce')
    d=d.dropna().drop_duplicates('ts').sort_values('ts').set_index('ts')
    if d.empty:
        return {}
    med=float(d.price.median())
    if not 100<med<10000:
        raise RuntimeError(f'XAUUSD price sanity check failed: {med}')

    out={}
    for tf,rule in [('M5','5min'),('M15','15min'),('H1','1h')]:
        o=d.price.resample(rule,label='left',closed='left').ohlc()
        o['volume']=d.price.resample(rule,label='left',closed='left').count()
        out[tf]=o.dropna()
    return out

def _merge_frames(old,new):
    if old.empty: return new.copy()
    if new.empty: return old.copy()
    x=pd.concat([old,new]).sort_index()
    return x[~x.index.duplicated(keep='last')]

def xau_frames():
    end=now().replace(minute=0,second=0,microsecond=0)
    cached={tf:_read_xau_cache(tf) for tf in ('M5','M15','H1')}

    # If all required caches exist and contain enough history, only refresh the
    # most recent rolling window. Otherwise bootstrap the full EMA200 warm-up.
    cache_ready=all(len(cached[tf]) >= EMA200+30 for tf in cached)

    if cache_ready:
        oldest_needed=end-timedelta(hours=XAU_INCREMENTAL_HOURS)
        fetch_start=oldest_needed.replace(minute=0,second=0,microsecond=0)
        log(f'XAU cache hit: {len(cached["H1"])} H1 candles; refreshing last {XAU_INCREMENTAL_HOURS} hours.')
    else:
        fetch_start=end-timedelta(hours=XAU_LOOKBACK_HOURS)
        log(f'XAU cache bootstrap: downloading {XAU_LOOKBACK_HOURS} hours with {DUKA_WORKERS} workers.')

    # Include the current hour plus a small overlap. The overlap lets us rebuild
    # the last closed candles cleanly without relying on a forming candle.
    hours=[]
    h=fetch_start
    while h<=end:
        hours.append(h)
    ...
    ticks=[]; errors=0
    # Even with workers enabled, keep concurrency deliberately low.
    with ThreadPoolExecutor(max_workers=DUKA_WORKERS) as ex:
        ...
        for f in as_completed(fs):
            ...
            try:
                ...
                errors += 1
                log(f'XAU hour {h:%Y-%m-%d %H}: {e}')

    if errors:
        log(f'XAUUSD: {errors} hour requests failed after retries.')

    fresh=_ticks_to_frames(ticks)
    if not fresh and not cache_ready:
        raise RuntimeError('Dukascopy returned no usable XAUUSD ticks during bootstrap.')

    # Merge fresh native candles into the persistent cache.
    for tf in ('M5','M15','H1'):
        cached[tf]=_merge_frames(cached[tf],fresh.get(tf,pd.DataFrame()))
        # Keep a bounded amount of history. H1 needs ~230 candles; 270 hours
        # gives the requested warm-up margin. M5/M15 retain the same time span.
        cutoff=end-timedelta(hours=max(XAU_LOOKBACK_HOURS, 300))
        cached[tf]=cached[tf][cached[tf].index>=cutoff]
        if len(cached[tf])>1:
            # Never expose a currently forming candle to the signal engine.
            cached[tf]=cached[tf].iloc[:-1] if cached[tf].index[-1]>=end else cached[tf]
        _write_xau_cache(tf,cached[tf])

        if len(cached[tf])<EMA200+30:
            raise RuntimeError(f'Not enough XAUUSD {tf} candles: {len(cached[tf])}')
