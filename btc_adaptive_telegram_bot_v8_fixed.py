...
def append_signal_csv(row):
    row=dict(row); row.setdefault("event","SIGNAL"); row.setdefault("scan_id",row.get("signal_id","")); append_scan_csv(row)
...
def run_scan(state,calibration):
    ...
    if any(len(F[k])<50 for k in ("5m","15m","1h")):journal_wait("insufficient_history",scan_id=sid); journal("SCAN_DONE",scan_id=sid,status="WAIT",reason="insufficient_history"); return
    ...
    if not signal:
        journal_wait("no_valid_signal",scan_id=sid,regime=market.get("regime"),confidence=market.get("confidence")); journal("SCAN_DONE",scan_id=sid,status="WAIT",reason="no_valid_signal"); return
