...
def _fusion_shadow_maturity(shadow):
    ...
    if status=="FAILED":
        # Shadow is evidence/risk context, not a standalone hard gate. A failed
        # shadow hypothesis must reduce fusion maturity, but it must not erase a
        # valid Core candidate by itself. Final Fusion risk/score/RR checks remain
        # authoritative and can still produce WAIT/BLOCK.
        return {"status":status,"maturity":0.0,"critical":False,"reason":"shadow_failed"}
...
def fusion_final_trade_gate(signal,F,market,adaptive_bridge=None,shadow_intel=None):
    ...
    out={"state":state,"reason":reason,"fusion_score":fusion_score,"shadow_status":maturity["status"],"shadow_maturity":maturity["maturity"],"shadow_reason":maturity["reason"],"core_gate_blocked":not core_allowed,"core_gate_reason":core_reason,"core_gate_recoverable":core_block_recoverable,"recovery_ready":recovery_ready,"bridge_ready":bridge_ready,"bridge_context":bridge_context,"bridge_relationship":bridge_rel,"bridge_factor":bridge_factor,"dynamic_factor":dynamic_factor,"core_factor":core_factor,"levels":levels,"rr":rr,"dynamic":dynamic,"version":FUSION_VERSION}
