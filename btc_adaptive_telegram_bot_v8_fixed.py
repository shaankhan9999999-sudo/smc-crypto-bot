...
STATE_HYSTERESIS_SCANS=2; REGIME_MIN_CONFIDENCE=.55; ADAPTIVE_MIN_SAMPLES=30; ADAPTIVE_STRONG_SAMPLES=75
...
def prepare(df):
    ...
    d["rsi"]=(100-100/(1+(up/dn.replace(0,np.nan)))).fillna(50); d["range"]=d.high-d.low
    d["ema21_slope_atr"]=(d["ema21"]-d["ema21"].shift(3))/d["atr"].replace(0,np.nan); d["ema21_slope_atr"]=d["ema21_slope_atr"].replace([np.inf,-np.inf],0).fillna(0)
    return d

def _ema21_state(d):
    if d.empty:return {"state":"UNKNOWN","distance_atr":0.0,"slope_atr":0.0,"accepted":False,"reclaimed":False}
    r=d.iloc[-1]; atr=max(safe_float(r.atr),safe_float(r.close)*.0001); close=safe_float(r.close); ema=safe_float(r.ema21); dist=(close-ema)/atr; slope=safe_float(r.ema21_slope_atr)
    prev_close=safe_float(d.iloc[-2].close) if len(d)>=2 else close
    prev_ema=safe_float(d.iloc[-2].ema21) if len(d)>=2 else ema
    if close>ema and prev_close<=prev_ema: state="RECLAIMED"
    elif close<ema and prev_close>=prev_ema: state="REJECTED"
    elif abs(dist)<=.20: state="TESTING"
    elif dist>.20 and slope>.03: state="ACCEPTED_LONG"
    elif dist<-.20 and slope<-.03: state="ACCEPTED_SHORT"
    elif dist>0: state="ABOVE"
    else: state="BELOW"
    accepted=state in {"ACCEPTED_LONG","ACCEPTED_SHORT"}
    return {"state":state,"distance_atr":dist,"slope_atr":slope,"accepted":accepted,"reclaimed":state=="RECLAIMED"}

def _price_structure_evidence(d):
    if len(d)<12:return {"score":.5,"bull":.5,"bear":.5,"bos_long":False,"bos_short":False,"choch_long":False,"choch_short":False,"ema21":_ema21_state(d)}
    r=d.iloc[-1]; atr=max(safe_float(r.atr),safe_float(r.close)*.0001); price=safe_float(r.close)
    prior=d.iloc[-9:-1]; older=d.iloc[-17:-9] if len(d)>=17 else d.iloc[:-9]
    ph=safe_float(prior.high.max(),price); pl=safe_float(prior.low.min(),price); oh=safe_float(older.high.max(),price); ol=safe_float(older.low.min(),price)
    bos_long=price>ph; bos_short=price<pl
    choch_long=(safe_float(prior.close.iloc[-1])-safe_float(prior.close.iloc[0])<0 and price>ph) if len(prior)>1 else False
    choch_short=(safe_float(prior.close.iloc[-1])-safe_float(prior.close.iloc[0])>0 and price<pl) if len(prior)>1 else False
    ema=_ema21_state(d); slope=clamp(.5+ema["slope_atr"]/.20); loc=clamp(.5+ema["distance_atr"]/1.5)
    long_break=clamp(.5+(price-ph)/(atr*1.5)); short_break=clamp(.5+(pl-price)/(atr*1.5))
    bull=clamp(.30*loc+.22*slope+.30*long_break+.10*bool(bos_long)+.08*bool(choch_long))
    bear=clamp(.30*(1-loc)+.22*(1-slope)+.30*short_break+.10*bool(bos_short)+.08*bool(choch_short))
    return {"score":bull,"bull":bull,"bear":bear,"bos_long":bool(bos_long),"bos_short":bool(bos_short),"choch_long":bool(choch_long),"choch_short":bool(choch_short),"ema21":ema,"recent_high":ph,"recent_low":pl,"older_high":oh,"older_low":ol}

def _tf_direction_score(df):
    d=prepare(df)
    if d.empty:return {"close":0,"atr":0,"trend":0,"structure":0,"momentum":0,"volume":0,"retest":0,"ema21_state":"UNKNOWN","ema21_slope":0.0}
    r=d.iloc[-1]; close=safe_float(r.close); atr=max(safe_float(r.atr),close*.0001); st=_price_structure_evidence(d); ema=st["ema21"]
    # EMA21 is now a location/state filter. Real structure (BOS/CHoCH/price acceptance) carries the structural signal.
    trend=clamp(.65*((r.ema9-r.ema50)/(atr*2)+.5)+.35*(.5+safe_float(r.ema21_slope_atr)/.20))
    return {"close":close,"atr":atr,"trend":trend,"structure":st["score"],
            "momentum":clamp(.5+r.ret4/.02),"volume":clamp(safe_float(r.volume)/(safe_float(r.vol_ma20) or 1)),
            "retest":clamp(.5-abs(close-r.ema21)/(atr*2)+.15*(ema["reclaimed"])),"rsi":safe_float(r.rsi,50),
            "ema21_state":ema["state"],"ema21_slope":ema["slope_atr"],"structure_detail":st}

def drop_open_candle(df,seconds):
    """Use only fully closed candles; the live forming candle cannot create repeated intrabar signals."""
    if not isinstance(df,pd.DataFrame) or df.empty:return df
    cutoff=int(time.time())-int(seconds)
    return df[df["ts"].astype(int)<=cutoff].reset_index(drop=True)
...
def _structure_transition(df,direction):
    ...
    ret=safe_float(d.iloc[-1].ret4); disp=clamp(abs(ret)/(atr/max(last,1e-9))); st=_price_structure_evidence(d); ema=st["ema21"]
    if direction=="LONG":
        bos=bool(st["bos_long"]); choch=bool(st["choch_long"]); structure=st["bull"]
    else:
        bos=bool(st["bos_short"]); choch=bool(st["choch_short"]); structure=st["bear"]
    # Structure is price-action first; EMA21 only confirms location/slope.
    structure=clamp(.78*structure+.22*(ema["distance_atr"]>0 if direction=="LONG" else ema["distance_atr"]<0))
    return {"score":structure,"bos":bos,"choch":choch,"displacement":disp,"recent_high":rh,"recent_low":rl,"ema21_state":ema["state"],"ema21_slope":ema["slope_atr"]}
...
def execution_score(F,direction):
    ...
    of=update_orderflow()
    if of.get("fresh") and of["buy"]+of["sell"]>0:parts["orderflow"]=clamp(.5+sign*of["delta"]/(of["buy"]+of["sell"])/2)
    else: parts["orderflow"]=.5
    score=sum(WEIGHTS[k]*clamp(v) for k,v in parts.items())
    if not of.get("fresh"):
        score-=WEIGHTS["orderflow"]*.35
    return {"score":max(0.0,score),"agreement":calculate_agreement(parts),"parts":parts,"orderflow":of,"orderflow_fresh":bool(of.get("fresh"))}
...
CSV_FIELDS=["ts","scan_id","event","reason","signal_id","direction","setup","regime","confidence","score","agreement","price","adaptive_adjustment","sweep_confirmation","active_trade","action","error"]
...
SIGNAL_CSV_FIELDS=["ts","scan_id","event","signal_id","direction","setup","regime","confidence","score","agreement","price","adaptive_adjustment","sweep_confirmation","flow_direction","flow_transition","flow_score","action","opportunity_size","bridge_state","bridge_strength"]
...
def update_orderflow():
    try:
        ...
        journal("ORDERFLOW_ERROR",error=repr(e))
    out=FLOW.copy(); out["fresh"]=bool(now_ts()-safe_float(out.get("ts"),0.0)<=ORDERFLOW_STALE and safe_float(out.get("ts"),0.0)>0)
    if not out["fresh"]:
        out["buy"]=out["sell"]=out["large_buy"]=out["large_sell"]=out["delta"]=0.0
        out["bids"]={}; out["asks"]={}
        journal("ORDERFLOW_STALE",age_seconds=max(0.0,now_ts()-safe_float(FLOW.get("ts"),0.0)),diagnostic_only=True)
    return out
...
def detect_continuation(df,direction):
    ...
    ema=_ema21_state(d)
    if direction=="LONG":imp=(current-sp)/atr; ext=safe_float(d.high.iloc[-CONT_LOOKBACK:].max()); pb=(ext-current)/max(abs(ext-sp),atr); trend=(current>r.ema21 and ema["slope_atr"]>=0) or ema["state"] in {"RECLAIMED","ACCEPTED_LONG"}; mom=r.ret4>0
    else:imp=(sp-current)/atr; ext=safe_float(d.low.iloc[-CONT_LOOKBACK:].min()); pb=(current-ext)/max(abs(sp-ext),atr); trend=(current<r.ema21 and ema["slope_atr"]<=0) or ema["state"] in {"REJECTED","ACCEPTED_SHORT"}; mom=r.ret4<0
    ok=CONT_MIN_PULLBACK<=pb<=CONT_MAX_PULLBACK; score=.40*clamp(imp/CONT_IMPULSE_ATR)+.25*ok+.20*bool(trend)+.15*bool(mom)
...
def shadow_support_resistance_diagnostics(F,scan_identifier=None,state=None):
    ...
    try:
        ...
        for e in events:
            ...
            event_payload={k:v for k,v in e.items() if k!="event"}
            journal("SHADOW_SR_EVENT",sr_event=event_name,structure_state=e.get("structure_state"),previous_state=e.get("previous_state"),structure_direction=e.get("structure_direction"),confirmation_age_scans=e.get("confirmation_age_scans"),**event_payload,diagnostic_only=True)
...
def create_trade_record(signal,levels):
    return {"status":"OPEN","signal_id":signal["signal_id"],"opened_at":utc_now().isoformat(),"direction":signal["direction"],"setup":signal["setup"],"regime":signal["regime"],
            ...
            "layer_authority":signal.get("layer_authority",{}),
...
# Explicit layer-authority map: evidence layers inform the decision, while only
# designated context/safety layers may veto it. This prevents hidden veto stacking.
LAYER_AUTHORITY={"REGIME":"HARD","FLOW_CONFLICT":"HARD","FLOW_MINIMUM":"HARD",
                 "TRANSITION_CONFIRMATION":"HARD","CORE_SCORE":"HARD","CORE_AGREEMENT":"HARD",
                 "RISK_RR":"HARD","SHADOW_FAILED":"HARD","SHADOW_MATURITY":"SOFT",
                 "EMA21":"SOFT","MTF":"EVIDENCE","STRUCTURE":"EVIDENCE","BOS_CHOCH":"EVIDENCE",
                 "RECLAIM_RETEST":"EVIDENCE","MOMENTUM":"EVIDENCE","VOLUME":"EVIDENCE",
                 "ORDERFLOW":"EVIDENCE","FOLLOW_THROUGH":"SOFT","ADAPTIVE_BRIDGE":"EVIDENCE","FUSION":"HARD"}

def layer_authority_snapshot(direction,market,execution,adaptive_bridge=None,shadow=None):
    flow=market.get("flow",{}) or {}; bridge=adaptive_bridge if isinstance(adaptive_bridge,dict) else {}; sh=shadow if isinstance(shadow,dict) else {}
    fd=str(flow.get("confirmed_direction","NEUTRAL") or "NEUTRAL")
    fs=safe_float(flow.get("long_score" if direction=="LONG" else "short_score"),.5)
    return {
        "REGIME":{"authority":LAYER_AUTHORITY["REGIME"],"value":str(market.get("regime","UNSTABLE"))},
        "FLOW_CONFLICT":{"authority":LAYER_AUTHORITY["FLOW_CONFLICT"],"value":fd,"conflict":fd not in {"NEUTRAL",direction}},
        "FLOW_MINIMUM":{"authority":LAYER_AUTHORITY["FLOW_MINIMUM"],"value":round(fs,4)},
        "CORE_SCORE":{"authority":LAYER_AUTHORITY["CORE_SCORE"],"value":round(safe_float(execution.get("score"),0),4),"pass":safe_float(execution.get("score"),0)>=MIN_SCORE},
        "CORE_AGREEMENT":{"authority":LAYER_AUTHORITY["CORE_AGREEMENT"],"value":round(safe_float(execution.get("agreement"),0),4),"pass":safe_float(execution.get("agreement"),0)>=MIN_AGREEMENT},
        "EMA21":{"authority":LAYER_AUTHORITY["EMA21"]},"MTF":{"authority":LAYER_AUTHORITY["MTF"]},
        "STRUCTURE":{"authority":LAYER_AUTHORITY["STRUCTURE"]},"BOS_CHOCH":{"authority":LAYER_AUTHORITY["BOS_CHOCH"]},
        "RECLAIM_RETEST":{"authority":LAYER_AUTHORITY["RECLAIM_RETEST"]},"MOMENTUM":{"authority":LAYER_AUTHORITY["MOMENTUM"]},
        "VOLUME":{"authority":LAYER_AUTHORITY["VOLUME"]},"ORDERFLOW":{"authority":LAYER_AUTHORITY["ORDERFLOW"]},
        "FOLLOW_THROUGH":{"authority":LAYER_AUTHORITY["FOLLOW_THROUGH"],"value":bool(bridge.get("follow_through"))},
        "ADAPTIVE_BRIDGE":{"authority":LAYER_AUTHORITY["ADAPTIVE_BRIDGE"],"state":bridge.get("bridge_state","NONE"),"ready":bool(bridge.get("bridge_ready"))},
        "SHADOW_MATURITY":{"authority":LAYER_AUTHORITY["SHADOW_MATURITY"],"value":str(sh.get("status","NONE"))},
        "SHADOW_FAILED":{"authority":LAYER_AUTHORITY["SHADOW_FAILED"],"failed":str(sh.get("status","NONE"))=="FAILED"},
        "FUSION":{"authority":LAYER_AUTHORITY["FUSION"]}
    }
...
def fusion_final_trade_gate(signal,F,market,adaptive_bridge=None,shadow_intel=None):
    ...
    if not decision.get("allowed"): return {"state":"BLOCK","reason":"core_decision_not_allowed","fusion_score":0.0}
    if score<MIN_SCORE or agreement<MIN_AGREEMENT: return {"state":"WAIT","reason":"core_score_or_agreement_below_min","fusion_score":0.0}
...
def build_signal(F,state,calibration,market,adaptive_intel=None,shadow_intel=None):
    ...
    for direction in ("LONG","SHORT"):
        ...
        shadow=(shadow_intel or {}).get(direction) if isinstance(shadow_intel,dict) else None
        layer_map=layer_authority_snapshot(direction,market,execution,adaptive_bridge,shadow)
        journal("LAYER_AUTHORITY_DIAGNOSTIC",direction=direction,regime=regime,layer_authority=LAYER_AUTHORITY,layer_map=layer_map,diagnostic_only=True)
        ...
        diagnostic_score=clamp(diagnostic_adaptive["score"]+diagnostic_setup_bonus+bridge_bonus,0,105)
        ...
        if not decision["allowed"]:
            ...
            journal("HARD_GATE_DIAGNOSTIC",direction=direction,regime=regime,regime_confidence=rc,setup=setup,gate=diagnostic_gate,gate_reason=diagnostic_gate_reason,gate_threshold=diagnostic_threshold,
                    ...
                    diagnostic_adaptive_score=diagnostic_adaptive.get("score"),diagnostic_setup_bonus=diagnostic_setup_bonus,adaptive_bridge=adaptive_bridge,bridge_bonus=bridge_bonus,bridge_agreement_boost=bridge_agreement_boost,
        ...
        # 4) market-derived evidence only: all entry evidence is auditable. Flow/OB do NOT get added again.
        setup_bonus=0.0
        ...
        score=clamp(adaptive["score"]+setup_bonus+bridge_bonus,0,105)
        ...
        fusion_signal={"direction":direction,"setup":setup,"regime":regime,"regime_confidence":rc,"score":score,"agreement":final_agreement,"base_score":execution["score"],"adaptive_score":adaptive["score"],"adaptive_adjustment":adaptive.get("adjustment",0),"sweep_confirmation":sc,"sweep":sweep,"continuation":cont,"parts":parts,"features":features,"price":price,"atr":atr,"flow":market.get("flow",{}),"flow_score":decision.get("flow_score",.5),"decision":decision,"order_block":sweep.get("order_block",{}),"adaptive_bridge":adaptive_bridge or {},"layer_authority":layer_map,"opportunity_size":(adaptive_bridge or {}).get("opportunity_size","SMALL")}
        ...
        candidates.append({"direction":direction,"setup":setup,"regime":regime,"regime_confidence":rc,"score":score,"base_score":execution["score"],"adaptive_score":adaptive["score"],
            ...
            "parts":parts,"features":features,"price":price,"atr":atr,
            "flow":market.get("flow",{}),"flow_score":decision.get("flow_score",.5),"decision":decision,"order_block":sweep.get("order_block",{}),
            "adaptive_bridge":adaptive_bridge or {},"layer_authority":layer_map,"opportunity_size":(adaptive_bridge or {}).get("opportunity_size","SMALL"),
...
def manage_active_trade(state,calibration,adaptive_intel=None):
    ...
    d=drop_open_candle(d,300)
    if d.empty:return None
    row=d.iloc[-1]; price=safe_float(row.close); hi=safe_float(row.high); lo=safe_float(row.low); atr=max(safe_float(row.atr),price*.0005); direction=trade["direction"]
    sl=safe_float(trade["stop_loss"]); tp=safe_float(trade["target"]); sl_hit=(lo<=sl) if direction=="LONG" else (hi>=sl); tp_hit=(hi>=tp) if direction=="LONG" else (lo<=tp)
    if sl_hit and tp_hit:
        # Candle OHLC cannot reveal intrabar ordering; use conservative SL-first treatment.
        journal("INTRABAR_BOTH_SL_TP",signal_id=trade.get("signal_id"),direction=direction,sl=sl,tp=tp,high=hi,low=lo,reason="ambiguous_candle_order")
        return close_trade(state,calibration,"SL_HIT_AMBIGUOUS_BAR",sl)
    if sl_hit:return close_trade(state,calibration,"SL_HIT_INTRABAR",sl)
    if tp_hit:return close_trade(state,calibration,"TARGET_HIT_INTRABAR",tp)
...
def record_signal(signal,action="SIGNAL"):
    row={"ts":utc_now().isoformat(),"scan_id":signal.get("signal_id"),"event":"SIGNAL","signal_id":signal.get("signal_id"),"direction":signal.get("direction"),"setup":signal.get("setup"),"regime":signal.get("regime"),
         ...
         "sweep_confirmation":signal.get("sweep_confirmation"),"flow_direction":signal.get("flow",{}).get("confirmed_direction"),"flow_transition":signal.get("flow",{}).get("transition"),"flow_score":signal.get("flow_score"),"action":action,"opportunity_size":signal.get("opportunity_size"),"bridge_state":signal.get("adaptive_bridge",{}).get("bridge_state"),"bridge_strength":signal.get("adaptive_bridge",{}).get("combined_strength")}; append_signal_csv(row); journal("SIGNAL",**row)
...
def run_scan(state,calibration):
    ...
    # Core decisions operate on closed candles only. This prevents repeated evaluation of the same live candle.
    F["1m"]=drop_open_candle(F["1m"],60) if USE_MICRO_CONFIRMATION else F["1m"]
    F["5m"]=drop_open_candle(F["5m"],300); F["15m"]=drop_open_candle(F["15m"],900); F["1h"]=drop_open_candle(F["1h"],3600)
    F["2h"]=drop_open_candle(F["2h"],7200); F["4h"]=drop_open_candle(F["4h"],14400)
    ...
    signal=build_signal(F,state,calibration,market,adaptive_intel,shadow_intel)
    print(f"V8_TRACE: build_signal_returned signal={bool(signal)}", flush=True)
    if not signal:
        journal_wait("no_valid_signal",scan_id=sid,regime=market.get("regime"),confidence=market.get("confidence"),flow_direction=market.get("flow",{}).get("direction"),flow_transition=market.get("flow",{}).get("transition"),flow_long=market.get("flow",{}).get("long_score"),flow_short=market.get("flow",{}).get("short_score")); journal("SCAN_DONE",scan_id=sid,status="WAIT",reason="no_valid_signal"); print("V8_TRACE: wait_no_signal_exit", flush=True); return
...
def initialize():
    state=load_state(); calibration=load_calibration(); journal("V8_ARCHITECTURE",version="V8-STRUCTURE-EVIDENCE-NO-NEWS",news_system="REMOVED",ema21_role="LOCATION_STATE_FILTER",core_closed_candles=True,intrabar_trade_management=True,orderflow_stale_guard=True,diagnostic_only=True); ensure_parent_dir(JOURNAL_FILE); ensure_parent_dir(CSV_LOG_FILE); ensure_parent_dir(SCAN_CSV_FILE); ensure_parent_dir(SHADOW_SR_FILE); ensure_parent_dir(SHADOW_INTEL_CSV); ensure_parent_dir(SHADOW_ORDERBOOK_CSV); ensure_parent_dir(ADAPTIVE_OPPORTUNITY_CSV); return state,calibration
def main():
