...
def _shadow_intel_opportunity(F,direction,state,scan_identifier,orderbook=None):
    ...
    try:
        ...
        evidence={
            ...
            "follow_through":bool(displacement>=SHADOW_INTEL_DISPLACEMENT_ATR),
...
def build_signal(F,state,calibration,market,news):
    ...
    for direction in ("LONG","SHORT"):
        ...
        diagnostic_score=clamp(diagnostic_adaptive["score"]+diagnostic_setup_bonus+diagnostic_news_bonus,0,105)
        ...
        if not decision["allowed"]:
            ...
            journal("HARD_GATE_DIAGNOSTIC",direction=direction,regime=regime,regime_confidence=rc,setup=setup,gate=diagnostic_gate,gate_reason=diagnostic_gate_reason,gate_threshold=diagnostic_threshold,
                    ...
                    diagnostic_adaptive_score=diagnostic_adaptive.get("score"),diagnostic_setup_bonus=diagnostic_setup_bonus,diagnostic_news_bonus=diagnostic_news_bonus,
...
def run_scan(state,calibration):
    ...
    # Diagnostic only; it never feeds decision_engine() or execution.
...
def initialize():
    state=load_state(); calibration=load_calibration(); ensure_parent_dir(JOURNAL_FILE); ensure_parent_dir(CSV_LOG_FILE); ensure_parent_dir(SCAN_CSV_FILE); ensure_parent_dir(SHADOW_SR_FILE); ensure_parent_dir(SHADOW_INTEL_CSV); ensure_parent_dir(SHADOW_ORDERBOOK_CSV); return state,calibration
def main():
