...
def run_smart_bot():
  print("=" * 60, flush=True)
  print("🔍 [SCAN START] Scanning BTC-USD 1-Hour chart for Smart Tiered Setups...", flush=True)
  print("=" * 60, flush=True)
  
  df = fetch_coinbase_klines("BTC-USD", granularity=3600)

  if df.empty or len(df) < 250:
    print("❌ Insufficient data from Coinbase.", flush=True)
    return
  ...
  print(f"📊 [MARKET STATS - 1H]", flush=True)
  print(f" • Current Price (Close) : {entry_price:.2f}", flush=True)
  print(f" • Candle Open / Close   : {latest['Open']:.2f} / {entry_price:.2f}", flush=True)
  print(f" • ATR (14)              : {current_atr:.2f}", flush=True)
  print(f" • Current Volume        : {latest['Volume']:.2f}", flush=True)
  print(f" • 10-H Volume Avg       : {latest['Volume_Avg']:.2f}", flush=True)
  print(f" • Volume OK (> Avg)     : {volume_ok}", flush=True)
  print(f" • EMA 9  (Prev/Latest)  : {prev['EMA_9']:.2f} / {latest['EMA_9']:.2f}", flush=True)
  print(f" • EMA 20 (Prev/Latest)  : {prev['EMA_20']:.2f} / {latest['EMA_20']:.2f}", flush=True)
  print(f" • EMA 21 (Prev/Latest)  : {prev['EMA_21']:.2f} / {latest['EMA_21']:.2f}", flush=True)
  print(f" • EMA 50 (Prev/Latest)  : {prev['EMA_50']:.2f} / {latest['EMA_50']:.2f}", flush=True)
  print(f" • EMA 200 (Prev/Latest) : {prev['EMA_200']:.2f} / {latest['EMA_200']:.2f}", flush=True)
  print("-" * 60, flush=True)

  # ==========================================
  # 1. A+ HIGH PROBABILITY SETUPS (EMA 200 Cross)
  # ==========================================
  print("🔎 Checking A+ High Probability Setups (EMA 200 Cross)...", flush=True)
  ...
  if bullish_cross_200 and volume_ok:
    print("🔥 A+ Bullish Golden Cross Conditions Met!", flush=True)
    ...
    if send_telegram_message(alert_message):
      print("📤 Telegram Alert Sent: A+ Bullish Cross", flush=True)
    return
  ...
  if bearish_cross_200 and volume_ok:
    print("🔻 A+ Bearish Death Cross Conditions Met!", flush=True)
    stop_loss = entry_price + (1.5 * current_atr)
    ...
    if send_telegram_message(alert_message):
      print("📤 Telegram Alert Sent: A+ Bearish Cross", flush=True)
    return

  # ==========================================
  # 2. STANDARD SETUPS (9/21 EMA Crossover + Trend)
  # ==========================================
  print("🔎 Checking Standard Setups (9/21 EMA Crossover + Trend)...", flush=True)
  ...
  if is_in_uptrend and ema_bullish_crossover and volume_ok:
    print("⚡ Standard Long Setup Conditions Met!", flush=True)
    stop_loss = entry_price - (1.5 * current_atr)
    risk = entry_price - stop_loss
    target_1 = entry_price + (2.0 * risk)
    ...
    if send_telegram_message(alert_message):
      print("📤 Telegram Alert Sent: Standard Long", flush=True)
    return
  ...
  if is_in_downtrend and ema_bearish_crossover and volume_ok:
    print("⚠️ Standard Short Setup Conditions Met!", flush=True)
    stop_loss = entry_price + (1.5 * current_atr)
    risk = stop_loss - entry_price
    target_1 = entry_price - (2.0 * risk)
    ...
    if send_telegram_message(alert_message):
      print("📤 Telegram Alert Sent: Standard Short", flush=True)
    return

  print("❌ No valid A+ or Standard setup found on this candle.", flush=True)
  print("=" * 60, flush=True)
