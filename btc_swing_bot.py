...
def run_swing_bot():
  print("=" * 60)
  print(
      "🔍 [SWING SCAN START] Scanning BTC-USD 15M Chart (Hybrid + Golden"
      " Crossover + Volume Filter)..."
  )
  print("=" * 60)

  df = fetch_coinbase_klines("BTC-USD", granularity=900)  # 15-Minute Candles

  if df.empty or len(df) < 220:  # 200 EMA ke liye sufficient data chahiye
    print(
        "❌ Insufficient data received from API (Need at least 220 candles for"
        " EMA 200)."
    )
    return
  ...
  # Volume Filter Condition (Current volume must be greater than average volume)
  ...
  print(
      f" • Volume / Avg Volume   : {current_vol:.2f} / {avg_vol:.2f}"
      f" (Confirmed: {volume_confirmed})"
  )
  print(
  ...
  # ==========================================
  # 1. HIGH PROBABILITY GOLDEN / DEATH CROSSOVER SETUP (With Volume Filter)
  # ==========================================
  golden_crossover = (
  ...
  if golden_crossover and volume_confirmed:
    print("🔥 HIGH PROBABILITY GOLDEN CROSSOVER DETECTED (Volume Confirmed)!")
    ...
    msg = (
        ...
        f"🎯 *Target:* `{target:.2f}`\n\n"
        f"💡 *Strategy Note:* Major Trend Shift Upwards. Recommended for"
        " **Heavy Lot Size**.\n"
        f"✅ RSI: {current_rsi:.1f} | ATR: {current_atr:.2f} | Vol Spike:"
        f" {current_vol:.1f}"
    )
  ...
  if death_crossover and volume_confirmed:
    print("🔥 HIGH PROBABILITY DEATH CROSSOVER DETECTED (Volume Confirmed)!")
    stop_loss = current_price + (2.5 * current_atr)
    target = current_price - (5.0 * current_atr)
    msg = (
        ...
        f"🛑 *Stop Loss:* `{stop_loss:.2f}`\n"
        f"🎯 *Target:* `{target:.2f}`\n\n"
        f"💡 *Strategy Note:* Major Trend Shift Downwards. Recommended for"
        " **Heavy Lot Size**.\n"
        f"✅ RSI: {current_rsi:.1f} | ATR: {current_atr:.2f} | Vol Spike:"
        f" {current_vol:.1f}"
    )
  ...
  # ==========================================
  # 2. SMART HYBRID LONG SETUP (With Volume Filter)
  # ==========================================
  ...
  if long_signal_triggered and volume_confirmed:
    print("🚀 Smart Hybrid Swing LONG Conditions Met (Volume Confirmed)!")
    ...
    msg = (
        ...
        f"✅ Volume Confirmed (Standard Lot)"
    )
    send_telegram_message(msg)
    return

  # ==========================================
  # 3. SMART HYBRID SHORT SETUP (With Volume Filter)
  # ==========================================
  ...
  if short_signal_triggered and volume_confirmed:
    print("📉 Smart Hybrid Swing SHORT Conditions Met (Volume Confirmed)!")
    stop_loss = current_price + (2.0 * current_atr)
    target = (
        support_level
  ...
  print(
      "❌ No valid swing setup generated in this cycle (or Volume filter"
      " restricted low-volume move)."
  )
  print("=" * 60)
