...
def run_ultimate_professional_scalper():
    print("Scanning BTC-USD on Coinbase (Smart Lookback + Detailed Debug Logs)...")
    ...
    # --- D. 3-Minute Liquidity Sweep Logic (Lookback window 25 candles) ---
    ...
    # --- F. Scanning recent 3 candles (Lookback Loop) ---
    for i in range(-3, 0):
        if abs(i) > len(df_1m):
            continue
        m1_row = df_1m.iloc[i]
    ...
    if bullish_setup:
        ...
        print("Smart Lookback Bullish Signal Sent to Telegram!")
        ...
        msg = (
            f"🩸 [SMART LOOKBACK SMC & VOLUME PROFILE SHORT]\n"
            f"Asset: BTC-USD (Coinbase)\n"
        ...
        send_telegram_message(msg)
        print("Smart Lookback Bearish Signal Sent to Telegram!")
    else:
        ...
        print(f"Sweep Long 3M: {sweep_long_3m} | Sweep Short 3M: {sweep_short_3m}")
