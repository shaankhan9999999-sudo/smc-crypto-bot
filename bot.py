...
def run_scalp_bot():
  ...
  # 1. FETCH 1-HOUR DATA FOR STRONG SUPPORT & RESISTANCE
  ...
  prior_1h = df_1h.iloc[
      -25:-1
  ]  # Pichle 1 ghante/din ke major candles ka range
  resistance_level = prior_1h["High"].max()
  support_level = prior_1h["Low"].min()

  # 2. FETCH 15-MINUTE DATA FOR TREND CONFIRMATION
  ...
  # 3. FETCH 5-MINUTE DATA FOR ENTRY & EXECUTION
  ...
  latest = df_5m.iloc[-2]  # Confirmed 5m closed candle
  prev = df_5m.iloc[-3]
  ...
  if volume_spike and volatility_ok:
    ...
    if (
      ...
      # Target naturally 1H resistance ya bada distance lega
      target = (
          resistance_level
